export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      fetchUpstreamModels: '从上游拉取模型',
      fetchingUpstreamModels: '拉取中...',
      fetchUpstreamModelsHint: '根据当前 Base URL 和 API Key 读取上游可用模型',
      fetchUpstreamModelsSuccess: '已从上游拉取 {count} 个模型',
      noUpstreamModelsFound: '上游未返回可用模型',
      fetchUpstreamModelsFailed: '从上游拉取模型失败',
      ...
      mappingRowIncomplete: '第 {line} 行映射不完整，请同时填写请求模型和实际模型',
      mappingDuplicateFromRow: '第 {line} 行请求模型 {model} 重复，请保留一条映射',
      whitelistWildcardNotSupported: '白名单第 {line} 行模型 {model} 不能包含通配符 *',
      wildcardOnlyAtEnd: '通配符 * 只能放在末尾',
      wildcardOnlyAtEndRow: '第 {line} 行请求模型通配符 * 只能放在末尾',
      targetNoWildcard: '目标模型不能包含通配符 *',
      targetNoWildcardRow: '第 {line} 行目标模型不能包含通配符 *',
      searchModels: '搜索模型...',
